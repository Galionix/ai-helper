  ...
  if (!document.getElementById("ai-helper-widget")) {
    ...
    const widget = el("div", {
      ...
      children: [
        ...
        el("div", { id: "ai-helper-chat", className: "ai-helper-content", textContent: "Привет! Я твой AI-помощник." }),
        el("div", {
          id: "ai-helper-settings", className: "ai-helper-content", style: { display: "none" },
          children: [el("label", { textContent: "API Key:" }), input, el("label", { textContent: "Цели:" }), goals]
        }),
        el("div", {
...
async function getAdvice(goals: string, recentEvents: string) {
  ...
  const messages = [
    {
      role: "system",
      content: "Ты персональный помощник, который анализирует поведение пользователя и напоминает ему о целях."
    },
    {
      role: "user",
...
async function maybeAdviseUser() {
  ...
  const now = Date.now();
  if (now - lastAdviceTime > 5 * 60 * 1000) {
    const raw = await GM_getValue("eventLog", "[]");
    const events = JSON.parse(raw).slice(-5); // последние 5 событий
